...
<?php $__env->startSection('js'); ?>
  ...
  <script type="text/javascript">
    ...
    $(function () {
      ...
      $("#enquiries").DataTable({
        ...
        "columns": [
          ...
          { "data": "verified" },
          {
            "data": "id",  // assuming each enquiry has an 'id'
            "render": function (data, type, row) {
              return `
              <button class="btn btn-sm btn-danger" onclick="deleteEnquiry(${data})">
    ...
    function deleteEnquiry(id) {
      ...
      var route = "<?php echo e(route('admin.manage-enquiries.destroy', ':id')); ?>";
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response.status === 200) {
            toastr.success(response.message || 'Enquiry deleted successfully.');
            $('#enquiries').DataTable().ajax.reload(null, false); // reload table without page refresh
          } else {
            toastr.error(response.message || 'Failed to delete enquiry.');
          }
          $(".loading").hide();
        },
        error: function (xhr) {
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (response.status === 200) {
            ...
            // ✅ Use optional chaining (?.) and fallbacks to avoid errors
            $(".listing_thumbnail").attr('src', prop.property_gallery?.[0]?.image_path
              ? "<?php echo e(config('app.url')); ?>/public/" + prop.property_gallery[0].image_path
              : "<?php echo e(asset('images/no-image.jpg')); ?>");

            $(".title").text(prop.title ?? 'N/A');
            $(".category").text(prop.category?.category_name ?? 'N/A');
            $(".subcategory").text(prop.sub_category?.sub_category_name ?? 'N/A');
            $(".subsubcategory").text(prop.sub_sub_category?.sub_sub_category_name ?? 'N/A');
            $(".location").text(prop.location?.location ?? 'N/A');
            $(".property_id").text(prop.listing_id ?? 'N/A');
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\web-mingo-project\prahit-properties\resources\views/admin/enquiries/index.blade.php ENDPATH**/ ?>