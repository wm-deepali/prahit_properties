<script src="<?php echo e(URL::asset('public/js/library/jquery-v3/jquery.js')); ?>"></script>
<script src="<?php echo e(URL::asset('public/js/library/jquery-v3/poppers.js')); ?>"></script>
<script src="<?php echo e(URL::asset('public/js/library/bootstrap-v4/bootstrap.js')); ?>"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-select/1.13.11/js/bootstrap-select.js"></script>
<script src="<?php echo e(URL::asset('public/js/library/owl-carousel-v2.3/owl.carousel.min.js')); ?>"></script>
...
<script src="<?php echo e(URL::asset('public/js/toastr.min.js')); ?>"></script>
...
<script type="text/javascript">
 function previewImage(input) {
    ...
    avatar.append('user_id', "<?php echo e(Auth::user()->id); ?>");
        $.ajax({
            url:"<?php echo e(url('user/upload_avatar')); ?>",
            method:"post",
            data:avatar,
                        datatype:'json',
                        cache: false,
            ...
            beforeSend:function(){
                $(".p-image").hide();
                $("#change_avatar").addClass('hw30 br0').attr('src', "<?php echo e(url('/').'/images/loading.gif'); ?>");
            },
            success:function(response){
                if(response.status === 200) {
                    toastr.success('Profile pic uploaded successfully');
                    $("#change_avatar").attr('src', "<?php echo e(url('/').'/'); ?>"+response.avatar_file).removeClass('hw30 br0');
                } else {
                    toastr.error('An error occured');
                    $("#change_avatar").attr('src', "<?php echo e(url('/').'/'.Auth::user()->avatar); ?>").removeClass('hw30 br0');
                }
            },
...
<script type="text/javascript">
    var path = "<?php echo e(url('home/autocomplete-search')); ?>";
    $('input.typeahead').typeahead({
        ...
        updater: function(item) {
            window.location = '<?php echo e(url('/')); ?>/'+item.name;
        }
    });
...
function claimListing(){
    ...
    $.ajax({
        url: "<?php echo e(url('property/claim')); ?>/"+id,
        method:'post',
        data: data,
        beforeSend:function(){
...
function verifyOTPForClaim() {
    ...
    data._token = $("input[name=_token]").val();
    $.ajax({
        url: "<?php echo e(url('claim/verified')); ?>",
        method:'post',
        data: data,
        beforeSend:function(){
            $(".loading").css('display','block');
...
function resendOTP() {
    $.ajax({
        url: "<?php echo e(url('claim/resend/otp')); ?>",
        method:'get',
        beforeSend:function(){
            $(".loading").css('display','block');
        },
...
</script><?php /**PATH /home/parhitproperties/public_html/parhit-2021/resources/views/layouts/front/app_js.blade.php ENDPATH**/ ?>