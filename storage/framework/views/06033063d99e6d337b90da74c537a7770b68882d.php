        ...
        <div class="modal-body">
          <form method="post" action="" id="transfer-files">
            ...
            <div class="row">
              ...
              <div class="col-sm-12">
                <center><button type="button" class="btn btn-primary" onclick="sendInformation()"
                    style="margin-top:15px;">Send</button></center>
              </div>
            </div>
          </form>
        </div>
...
<?php $__env->startSection('js'); ?>
  <script type="text/javascript">
    ...
    function getApprovedPropertiesDatatable(queryParams = '?') {
      var route = '<?php echo e(route('admin.manageApprovedPropertiesDatatable')); ?>' + queryParams;
      ...
      swal.fire({
        ...
        .then((willDelete) => {
          if (willDelete) {
            ...
            $(".btn-delete").attr('disabled', true);
            $.ajax({
              url: '<?php echo e(route('admin.publishedProperty')); ?>',
              method: "POST",
              data: {
                "_token": "<?php echo e(csrf_token()); ?>",
                'id': id
    ...
    function downloadZipFiles(id) {
      $.ajax({
        url: '<?php echo e(url('create/property-images/zip')); ?>/' + id,
        method: "get",
        data: {
          "_token": "<?php echo e(csrf_token()); ?>",
          'id': id
            ...
            if (status == 'Unpublish') {
              ...
              $.each(response.data.Property.property_gallery, function (key, image) {
                $("#render-images").append(`<div class="col-sm-3">
                                              <img src="<?php echo e(config('app.url')); ?>/${image.image_path}" style="height:50px;" />
                                            </div>`);
              });
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\web-mingo-project\prahit-properties\resources\views/admin/properties/approved_properties.blade.php ENDPATH**/ ?>