	...
	public function property_detail($id, $slug)
	{
		...
		if ($property_detail->additional_info) {
			foreach (json_decode($property_detail->additional_info, true) as $field) {
				...
				if (!empty($field['label']) && isset($field['userData'])) {
					...
					if (is_array($field['userData'])) {
						...
						$values = array_filter($field['userData'], function ($value) {
							return $value !== '--select--' && trim($value) !== '';
						});

						if (!empty($values)) {
							$features[$cleanLabel] = implode(', ', $values);
						}

					} else {

						$value = trim($field['userData']);
