...
class AdsManagementController extends AppController {

	public function index() {
		$ads = Ads::has('Property')->with('Property')->latest()->get();
	...
	public function store(Request $request) {
		...
		try {
			if($request->has('thumbnail_file')) {
				$file = $this->fileUpload($request, ['uploads/ads' => 'thumbnail_file']);
				$request['thumbnail'] = isset($file[0]) ? $file[0] : '';
			}

			$adObj = Ads::create($request->all());
			if($adObj->exists()){
				$this->JsonResponse(200, 'Ad created successfully', []);
			} else {
