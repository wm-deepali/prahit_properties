...
{
    /**
     * Display all agent profile reviews.
     */
    public function index()
    {
        // Load only reviews that have a related user
        $reviews = Review::with(['profileSection', 'user'])
            ->whereHas('profileSection.user') // ensures user exists
            ->latest()
            ->paginate(10);

        return view('admin.agent-reviews.index', compact('reviews'));
    }
    ...
    {
        $review = Review::with(['profileSection', 'user'])->findOrFail($id);
        return response()->json($review); // ✅ JSON Response
    }
    ...
    {
        $reviews = Review::with(['profileSection', 'user'])
            ->whereHas('profileSection', function ($q) {
                $q->where('user_id', auth()->id());
            })
            ->latest()
            ->paginate(10);

        return view('front.user.agent-profile-review', compact('reviews'));
    }
    ...
    public function sentReviews()
    {
        ...
        $agentReviews = Review::with(['user', 'profileSection'])
        ->whereHas('profileSection.user') // ensures user exists
            ->where('user_id', $user->id)
            ->get();

        $businessReviews = \App\Models\BusinessListingReview::with(['user', 'businessListing'])
            ->where('user_id', $user->id)
            ->get();

        return view('front.user.sent-reviews', compact('agentReviews', 'businessReviews'));
    }
}
