...
class PropertiesController extends AppController
{
	...
	public function create() {
		...
		return view('admin.properties.create', compact('category','locations','form_type', 'states', 'amenities'));
	}
	...
	public function edit($id) { 
		...
		$amenities = Amenity::where('status', 'Yes')->get();
		return view('admin.properties.edit', compact('property','category','locations', 'states', 'cities', 'sub_locations', 'amenities'));
	}
	...
	public function update(Request $request) {
		...
		try {
			...
			if($request->has('property_use_for')) {
				$request['property_use_for'] = implode(',', $request->property_use_for);
			}
			if($request->has('amenties_features')) {
				$request['amenties_features'] = implode(',', $request->amenties_features);
			}
			if($request->has('feature_image_file')) {
				$feature_image = $this->fileUpload($request, ['uploads/properties/feature_image/' => 'feature_image_file']);
				$request['featured_image'] = isset($feature_image) ? $feature_image[0] : '';
			...
			if($properties->update($request->all())) {
				// PropertiesFields::where('property_id', $properties->id)->delete();
				// $listing_features = json_decode($request->listing_features, true);
				// $propertiesFields;
				// foreach ($listing_features as $key => $value) {
				// 	if(strlen($value) > 1) {
				// 		$propertiesFields = PropertiesFields::create(['property_id' => $properties->id, 'formtype_id' => $request->formtype_id, 'sub_feature_id' => $key, 'feature_value' => $value]);
				// 	}
				// }
				...
				if($request->has('gallery_images_file')) {
					$gallery_images = $this->multipleFileUpload($request, ['uploads/properties/gallery_images/' => 'gallery_images_file']);
					foreach ($gallery_images as $key => $value) {
						PropertyGallery::create(['property_id' => $properties->id, 'image_path' => $value]);					
					}
				}
				$this->JsonResponse(200, 'Property updated successfully', ['listing' => $properties]);
			} else {
				$this->JsonResponse(400, 'An error occured.');
			}
		} catch(\Exception $e) {
	...
	public function propertyDataFilter(Request $request) {
		$properties = Properties::with(['PropertyTypes','PropertyGallery','Category',
				'Category.SubCategory','Location','getUser', 'getCity', 'getState'])->select('*', \DB::raw('DATE_FORMAT(created_at, "%d %M, %Y") as formatted_date'));
