...
Trait GlobalTrait {
    ...
    */
    ...
    public function sendSMSInformtaion($mobile_number, $message) {
        try {
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL,"http://sms.webmingo.in/api/sendhttp.php?authkey=326070AyKV9eSm5e9545e4P1&mobiles=$mobile_number&message=$message&sender=BHOOMI&route=4&country=91");
            curl_setopt($ch, CURLOPT_POST, 1);
            // Receive server response ...
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            $server_output = curl_exec($ch);
    ...
    protected function imageUpload($data, $key, $old_file = null, $path) {
        if($data->$key) {
            ...
            base_path() . '/public/payment_screenshots/', $imageName
            );
