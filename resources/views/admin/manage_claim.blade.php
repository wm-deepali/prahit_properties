  ...
  <script type="text/javascript">
    ...
    $(function () {
      var table = $('#states').DataTable({
        ...
        render: true,
        searching: true,
        ajax: "{{ route('admin.manageClaimsDatatable') }}",
        columns: [
        ...
        success: function (response) {
          ...
          if (response.status === 200) {
            ...
            if (data.property_gallery && data.property_gallery.length > 0 && data.property_gallery[0].image_path) {
              $(".listing_thumbnail").attr('src', "{{ config('app.url') }}/public/" + data.property_gallery[0].image_path);
            } else {
              $(".listing_thumbnail").attr('src', "{{ asset('images/default-thumbnail.png') }}"); // fallback image
            }
