                      ...
                      @if(isset($locations) && count($locations) > 0)
                        @foreach($locations as $k => $v)
                          <tr id="{{$v->id}}">
                            ...
                            <td>{{$v->location}}</td>
                            <td><a
                                href="{{route('admin.edit_sublocation', ['id' => base64_encode($v->id)])}}">{{$v->sub_locations_count}}</a>
                            </td>
                            <td>
                              @if($v->status == "0")
                                Active
                            ...
                            <td>
                              <ul class="action">
                                <li><a href="{{route('admin.edit_sublocation', ['id' => base64_encode($v->id)])}}"
                                ...
                                <li><a href="#" data-toggle="modal" data-target="#delete-locations"
                                    onclick="$('#delete_locations #id').val({{$v->id}})"><i class="fas fa-trash"></i></a></li>
                              </ul>
                            </td>
                          </tr>
                        @endforeach
  ...
  <script type="text/javascript">
    $(function () {
      ...
      $("#update_locations").validate({
        submitHandler: function () {
          var route = "{{route('admin.locations.update', ['location' => ':id'])}}";
      ...
      $(".btn-delete").on('click', function (e) {
        ...
        var route = "{{route('admin.locations.destroy', ['location' => ':id'])}}";
    ...
    function fetchData(id) {
      document.getElementById('new_loader').style.display = 'block';
      var route = "{{route('admin.locations.show', ':id')}}";
      var route = route.replace(':id', id);
