...
@endphp
    ...
    <div class="position-relative rounded-4 overflow-hidden shadow-lg">
        ...
        {{-- Main Image --}}
        <a href="{{ $mainImage }}" data-lightbox="property-gallery">
            <img src="{{ $mainImage }}"
        ...
        <div class="position-absolute top-0 end-0 m-3">
            <span class="badge bg-dark px-3 py-2 fs-6">
                ...
                {{ $property_detail->PropertyGallery->count() }} Photos
            </span>
        </div>
    </div>

    {{-- Hidden Gallery Images --}}
    @foreach($property_detail->PropertyGallery as $gallery)
...
<div class="col-md-8">
    ...
    <div class="property-location p-3">
        ...
        <div class="mt-3 p-3 bg-light rounded-3">
            ...
            @if($property_detail->landmark)
                <p class="mb-1 text-muted">
                    <strong>Landmark:</strong> {{ $property_detail->landmark }}
                </p>
            @endif
            ...
            <p class="mb-0 text-muted">
                {{ $property_detail->getCity?->name }},
                {{ $property_detail->getState?->name }}
                – {{ $property_detail->pincode }}
            </p>
        </div>

        {{-- GET DIRECTIONS --}}
        @if($property_detail->latitude && $property_detail->longitude)
            <a href="https://www.google.com/maps?q={{ $property_detail->latitude }},{{ $property_detail->longitude }}"
               target="_blank"
...
const areaUnitMap = {
    ...
    SECTION_ORDER.forEach(section => {
        ...
        if (section === 'furnishing status') {
            const description = @json($property_detail->description);
            if (description) {
                html += `
                    <div class="col-sm-12 mb-2 mt-4">
                        <h4 style="color:#2c8c56;font-size:24px;font-weight:700;
