          ...
          <ul class="nav nav-tabs" id="myTab" role="tablist">
            ...
            @if(isset($category))
              @foreach($category as $a=>$b)
                <li class="nav-item"> <a class="nav-link {{$a==0 ? 'active' : ''}}" href="#" aria-selected="false">{{$b->category_name}}</a> </li>
              @endforeach
            @endif
            ...
            <div class="city-main text-center">
                ...
                @endphp 
      		      <a href="{{ url('/') }}/{{ $get_city->name }}">
              		<div class="thumb"> <img class="img-fluid" src="{{ asset('storage') }}/{{ $popular_city->image }}" alt="pc1.png"> </div>
              		<div class="details">
    					...
    					<div class="property-list-01">
    						<div class="property-img">
    							<a href="{{route('property_detail', ['title' => $value->slug])}}"> 
    								<img src="{{isset($value->PropertyGallery[0]->image_path) ?  asset('').$value->PropertyGallery[0]->image_path : ''}}" class="img-fluid"> <span class="type-pro">{{isset($value->property_types->type) ? $value->property_types->type : ''}}</span><span class="price-pro">Rs. {{number_format($value->price, 2)}}</span> 
    							</a>
    						</div>
				...
				<div class="col-lg-4 mb-3">
					<div class="project-main">
						...
						<div class="image-proj">
							<img src="{{isset($trending->PropertyGallery[0]->image_path) ?  asset('').$trending->PropertyGallery[0]->image_path : ''}}" class="img-fluid">
						</div>
						
						<div class="info-proj">
							<h4 class="proj-name">{{ $trending->title }}</h4>
							<span class="apart-name">{{ \Illuminate\Support\Str::limit($trending->description, 100, $end='...') }}</span>
							<span class="apart-adress">{{ $trending->getState ? $trending->getState->name : '' }}, {{ $trending->getCity ? $trending->getCity->name : '' }}, {{ $trending->Location ? $trending->Location->name : '' }}, {{ $trending->address }}</span>
	    	...
	    	@foreach($listings as $key=>$value)
		    	...
		    	<div class="col-sm-2">
			        <div class="property-gridsm">
			          	<div class="property-img"><a href="property-details.php"> <img src="{{isset($value->PropertyGallery[0]->image_path) ?  asset('').$value->PropertyGallery[0]->image_path : ''}}" class="img-fluid"> <span class="type-pro">{{isset($value->property_types->type) ? $value->property_types->type : ''}}</span><span class="price-pro">RRs. {{number_format($value->price, 2)}}</span> </a>
			          	</div>
			          	<div class="property-content">
			           		<div class="property-title">
			             		<h4><a href="property-details.php">{{$value->title}}</a></h4>
			              		<a href="#" class="property-address"> <i class="fas fa-map-marker"></i> {{$value->address}} </a>
			              	</div>
				        </div>
			        </div>
					...
					<div class="property-list-3">
						...
						<div class="listing-img-wrapper">
							<a href="{{route('property_detail', ['title' => $featured->slug])}}">
								<img src="{{isset($featured->PropertyGallery[0]->image_path) ?  asset('').$featured->PropertyGallery[0]->image_path : ''}}" class="img-fluid mx-auto" alt="">
							</a>
						...
						<div class="listing-content">
							<div class="listing-detail-wrapper-box">
								<div class="listing-detail-wrapper">
									<div class="listing-short-detail">
										...
										<span class="prt-types sale">{{ \Illuminate\Support\Str::limit($featured->description, 100, $end='...') }}</span>
									</div>
							...
							<div class="listing-footer-wrapper">
								<div class="listing-locate">
									<span class="listing-location"><i class="fas fa-map-marker-alt"></i>{{ $featured->getState ? $featured->getState->name : '' }}, {{ $featured->getCity ? $featured->getCity->name : '' }}, {{ $featured->Location ? $featured->Location->name : '' }}, {{ $featured->address }}</span>
								</div>
			        ...
			        <div class="item">
			            <article class="quote-modern">
			              	<div class="quote-modern-inner">
				                <time class="quote-modern-time" datetime="2020">{{date("jS F, Y", strtotime($testimonial->created_at))}}</time>
				                <div class="quote-modern-main">
				                  	<p>{{ $testimonial->description }}</p>
				                </div>
				                <div class="quote-modern-meta-outer">
				                	<img class="quote-modern-avatar" src="{{ asset('storage') }}/{{ $testimonial->image }}" alt="" width="57" height="57" /> 
